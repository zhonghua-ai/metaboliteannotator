...
extract_vaniya_fiehn_info <- function(block) {
  # Skip empty blocks
...
# Function to process Vaniya-Fiehn file
process_vaniya_fiehn_file <- function(input_file, output_file) {
  ...
  result_list <- furrr::future_map(blocks, extract_vaniya_fiehn_info, .progress = TRUE)
...
msp_file <- "/Users/zhonghua/Documents/Shimazu_Test/msdial_Expert/database/MoNA-export-Vaniya-Fiehn_Natural_Products_Library.msp"
...
output_file <- file.path(output_dir, "processed_vaniya_fiehn_compounds.fst")
vaniya_fiehn_result <- process_vaniya_fiehn_file(msp_file, output_file)

# Print summary statistics
cat("\nProcessing Summary:\n")
cat(sprintf("Total compounds: %d\n", vaniya_fiehn_result$total_compounds))
cat(sprintf("Unique compounds: %d\n", vaniya_fiehn_result$unique_compounds))
cat(sprintf("Output saved to: %s\n", vaniya_fiehn_result$output_file)) 
