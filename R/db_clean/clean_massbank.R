...
extract_massbank_info <- function(block) {
  # Skip empty blocks
...
# Function to process MassBank file
process_massbank_file <- function(input_file, output_file) {
  ...
  result_list <- furrr::future_map(blocks, extract_massbank_info, .progress = TRUE)
...
msp_file <- "/Users/zhonghua/Documents/Shimazu_Test/msdial_Expert/database/MoNA-export-MassBank.msp"
...
output_file <- file.path(output_dir, "processed_massbank_compounds.fst")
massbank_result <- process_massbank_file(msp_file, output_file)

# Print summary statistics
cat("\nProcessing Summary:\n")
cat(sprintf("Total compounds: %d\n", massbank_result$total_compounds))
cat(sprintf("Unique compounds: %d\n", massbank_result$unique_compounds))
cat(sprintf("Output saved to: %s\n", massbank_result$output_file)) 
